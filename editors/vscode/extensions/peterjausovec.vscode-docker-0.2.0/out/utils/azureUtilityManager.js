...
class AzureUtilityManager {
    ...
    getContainerRegistryManagementClient(subscription) {
        let client = new azure_arm_containerregistry_1.ContainerRegistryManagementClient(this.getCredentialByTenantId(subscription.tenantId), subscription.subscriptionId);
        vscode_azureextensionui_1.addExtensionUserAgent(client);
        return client;
    }
    getResourceManagementClient(subscription) {
        return new azure_arm_resource_1.ResourceManagementClient(this.getCredentialByTenantId(subscription.tenantId), subscription.subscriptionId);
    }
