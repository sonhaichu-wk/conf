...
async function deleteAzureRegistry(context) {
    ...
    if (shouldDelete) {
        let subscription = acrTools.getSubscriptionFromRegistry(registry);
        let resourceGroup = acrTools.getResourceGroupName(registry);
        const client = azureUtilityManager_1.AzureUtilityManager.getInstance().getContainerRegistryManagementClient(subscription);
