...
class RootNode extends nodeBase_1.NodeBase {
        ...
        try {
            ...
            for (let i = 0; i < images.length; i++) {
                ...
                if (!images[i].RepoTags) {
                    let node = new imageNode_1.ImageNode(`<none>:<none>`, this.eventEmitter);
                    ...
                    for (let j = 0; j < images[i].RepoTags.length; j++) {
                        // tslint:disable-next-line:prefer-for-of // Grandfathered in
                        let node = new imageNode_1.ImageNode(`${images[i].RepoTags[j]}`, this.eventEmitter);
                        node.imageDesc = images[i];
                        imageNodes.push(node);
                    }
                }
    ...
    async getRegistries() {
        ...
        registryRootNodes.push(new registryRootNode_1.RegistryRootNode('Docker Hub', "dockerHubRootNode", null));
        ...
        registryRootNodes.push(new registryRootNode_1.RegistryRootNode('Private Registries', 'customRootNode', null));
