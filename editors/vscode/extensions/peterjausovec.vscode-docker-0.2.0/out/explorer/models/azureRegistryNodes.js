...
class AzureRepositoryNode extends nodeBase_1.NodeBase {
    ...
    async getChildren(element) {
        const imageNodes = [];
        let node;
        let repo = new repository_1.Repository(element.registry, element.label);
        let images = await acrTools_1.getImagesByRepository(repo);
        for (let img of images) {
            node = new AzureImageTagNode(element.azureAccount, element, img.subscription, img.registry, img.registry.loginServer, img.repository.name, img.tag, img.created);
