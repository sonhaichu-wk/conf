...
class RegistryRootNode extends nodeBase_1.NodeBase {
    ...
    async getDockerHubOrgs() {
        ...
        if (extensionVariables_1.ext.keytar) {
            id.token = await extensionVariables_1.ext.keytar.getPassword(constants_1.keytarConstants.serviceId, constants_1.keytarConstants.dockerHubTokenKey);
            id.username = await extensionVariables_1.ext.keytar.getPassword(constants_1.keytarConstants.serviceId, constants_1.keytarConstants.dockerHubUserNameKey);
            id.password = await extensionVariables_1.ext.keytar.getPassword(constants_1.keytarConstants.serviceId, constants_1.keytarConstants.dockerHubPasswordKey);
        }
        if (!id.token) {
        ...
        namespaces.forEach((namespace) => {
            let node = new dockerHubNodes_1.DockerHubOrgNode(`${namespace}`);
        ...
        if (loggedIntoAzure) {
            ...
            for (let sub of subscriptions) {
                subPool.addTask(async () => {
                    const client = azureUtilityManager_1.AzureUtilityManager.getInstance().getContainerRegistryManagementClient(sub);
                    try {
                ...
                for (let j = 0; j < registries.length; j++) {
                    if (!registries[j].sku.tier.includes('Classic')) {
                        regPool.addTask(async () => {
                            let node = new azureRegistryNodes_1.AzureRegistryNode(registries[j].loginServer, this._azureAccount, registries[j], subscription);
