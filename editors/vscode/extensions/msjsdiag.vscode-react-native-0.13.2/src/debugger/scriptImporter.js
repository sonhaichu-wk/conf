...
class ScriptImporter {
    ...
    downloadAppScript(scriptUrlString, projectRootPath) {
        ...
        return request_1.Request.request(overriddenScriptUrlString, true).then(scriptBody => {
            return reactNativeProjectHelper_1.ReactNativeProjectHelper.getReactNativeVersions(projectRootPath).then(rnVersions => {
                ...
                if (reactNativeProjectHelper_1.ReactNativeProjectHelper.getRNVersionsWithBrokenMetroBundler().indexOf(rnVersions.reactNativeVersion) >= 0) {
    ...
    downloadDebuggerWorker(sourcesStoragePath, projectRootPath, debuggerWorkerUrlPath) {
        ...
        return packagerStatus_1.ensurePackagerRunning(this.packagerAddress, this.packagerPort, errPackagerNotRunning)
            .then(() => {
            return reactNativeProjectHelper_1.ReactNativeProjectHelper.getReactNativeVersions(projectRootPath);
        })
            .then((rnVersions) => {
