...
class SettingsHelper {
     ...
     * We get the packager port configured by the user
     */
    static getPackagerStatusIndicatorPattern(fsPath) {
        const projectRoot = SettingsHelper.getReactNativeProjectRoot(fsPath);
        let uri = vscode.Uri.file(projectRoot);
        const workspaceConfiguration = vscode.workspace.getConfiguration("react-native.packager", uri);
        if (workspaceConfiguration.has("status-indicator")) {
            const version = configurationReader_1.ConfigurationReader.readString(workspaceConfiguration.get("status-indicator"));
            if (version === packagerStatusIndicator_1.PackagerStatusIndicator.FULL_VERSION || version === packagerStatusIndicator_1.PackagerStatusIndicator.SHORT_VERSION) {
                return version;
            }
        }
        return packagerStatusIndicator_1.PackagerStatusIndicator.FULL_VERSION;
    }
}
